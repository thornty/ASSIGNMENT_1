...
demo_train <- demo_train %>%  
    select(SUBJ, VISIT, Ethnicity, Diagnosis, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
...
part_merge <- merge(demo_train, token_train, by.x=c("SUBJ","VISIT"), by.y=c("SUBJ","VISIT"), all = T)
full_merge <- merge(part_merge, LU_train, by.x=c("SUBJ", "VISIT"), by.y=c("SUBJ", "VISIT"), all = T)
...
full_data <- full_merge %>%
  left_join(temp %>% filter(VISIT == 1), by = c("SUBJ", "VISIT")) %>%
  mutate(
    MullenRaw1 = ifelse(VISIT == 1, MullenRaw1, MullenRaw),
    ExpressiveLangRaw1 = ifelse(VISIT == 1, ExpressiveLangRaw1, ExpressiveLangRaw),
    Socialization1 = ifelse(VISIT == 1, Socialization1, Socialization),
    ADOS1 = ifelse(VISIT == 1, ADOS1, ADOS)
  ) %>%
  fill(MullenRaw1, ExpressiveLangRaw1, Socialization1, ADOS1) 
...
write.csv(full_data, "ASD_TD_lang.csv")
```
