...
```{r setup}
### Loading in the tidyverse using pacman, install pacman and tidyverse if necessary
...
#assuming that data is placed in working directory
demographics <- read_csv("demo_train.csv")
LUs <- read_csv("LU_train.csv")
...
```{r rename_variables}
#all three dfs have the a variation of subject in the first column and visit in the second column. Names are set to be identical
LUs <- rename(LUs, subject = 1, visit = 2)
demographics <- rename(demographics, subject = 1, visit = 2)
words <- rename(words, subject = 1, visit = 2)
```
...
```{r extract_digits}
### extracting digits
LUs$visit <- str_extract(LUs$visit, "\\d")
words$visit <- str_extract(words$visit, "\\d")
```
...
# LUs$subject <- str_replace_all(LUs$subject, "\\.", "")
# words$subject <- str_replace_all(words$subject, "\\.", "")
# demographics$subject <- str_replace_all(demographics$subject, "\\.", "")
...
```{r}
#Create list of dfs
df_list <- list(demographics, LUs, words)

#define function to remove "." from subject column
dot_removal_func <- function(df){
    df <- df %>% 
        mutate(subject = str_replace_all(subject, "\\.", ""))
}

#overwrite df_list with list of modified dfs where dots have been removed by applying the above defined funciton
df_list <- map(df_list, dot_removal_func)
...
#Vector of target col names
variables_of_interest <- c("subject", "visit", "Diagnosis", "Ethnicity", "Gender", "Age", "ADOS",  "MullenRaw", "ExpressiveLangRaw", "Socialization", "MOT_MLU", "CHI_MLU", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI")



#def function to select variables present in list and given df
var_of_int_func <- function(df) {
    
    df <-  df %>% 
        select(intersect(colnames(df), variables_of_interest))
} 


#apply variable selection func across list
df_list <- map(df_list, var_of_int_func)
...
```{r merge_data}
#first unlist modified dfs from list
names(df_list) <- c("demographics", "LUs", "words")
list2env(df_list, envir = .GlobalEnv)

#vist column to doubles
LUs$visit <- as.double(LUs$visit)
words$visit <- as.double(words$visit)
...
### Write your code here
first_visit_df <- merged_df %>%
    filter(visit == 1)

#update df
merged_df <- merged_df %>% 
    
    #for each combination of following parameters (only subject) create the following new coloumns
    group_by(subject) %>% 
    
    mutate(
        #Create a new column for ADOS measurement at visit 1. Which creates new column with values for given subject at first visit
        ADOS1 = ADOS[which(visit==1)],
        MullenRaw1 = MullenRaw[which(visit==1)],
        ExpressiveLangRaw1 = ExpressiveLangRaw[which(visit==1)],
        Socialization1 = Socialization[which(visit==1)])
...
```{r revert_encoding}
### reverse gender and Diagnosis encoding
merged_df <- merged_df %>% 
    mutate(
        Gender = if_else(Gender == "1", "M", "F"),
...
write_csv(merged_df, "cleaned_data.csv")
