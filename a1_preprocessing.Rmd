...
```{r setup}
pacman::p_load(stringr, dplyr, tidyverse)
```
...
len_utt <- read_csv('token_train.csv')
word_data <- read_csv('LU_train.csv')
```
...
```{r extract_digits}
len_utt$VISIT <- str_extract(len_utt$VISIT, "\\d")
...
len_utt$SUBJ <- str_replace_all(len_utt$SUBJ, "[.]","")
...
```{r merge_data}
merged_first = merge(demo_data, len_utt, by.x=c('SUBJ', 'VISIT'), by.y=c('SUBJ', 'VISIT'), all=TRUE)
full_data = merge(merged_first, word_data, by.x=c('SUBJ', 'VISIT'), by.y=c('SUBJ', 'VISIT'), all=TRUE)
```
...
temp1 <- temp %>% 
    filter(VISIT == 1)
final_temp <- merge(temp1, temp, by.x=c('SUBJ'), by.y=c('SUBJ'), all.x = TRUE)
final_temp <- final_temp %>% 
    select(SUBJ, VISIT.y, ADOS1.x, MullenRaw1.x, ExpressiveLangRaw1.x, Socialization1.x)
final_temp <- final_temp %>% 
    rename(VISIT = VISIT.y, ADOS1 = ADOS1.x, MullenRaw1 = MullenRaw1.x, ExpressiveLangRaw1 = ExpressiveLangRaw1.x, Socialization1 = Socialization1.x)
full_data_set <- merge(full_data, final_temp, by.x=c('SUBJ', 'VISIT'), by.y=c('SUBJ', 'VISIT'), all=TRUE)
```
...
```{r revert_encoding}
# first resolve gender 1= M, 2= F
full_data_set$Gender <- replace(full_data_set$Gender, full_data_set$Gender==1, "M")
full_data_set$Gender <- replace(full_data_set$Gender, full_data_set$Gender==2, "F")
full_data_set$Diagnosis <- replace(full_data_set$Diagnosis, full_data_set$Diagnosis=="A", "ASD")
full_data_set$Diagnosis <- replace(full_data_set$Diagnosis, full_data_set$Diagnosis=="B", "TD")
```
...
```{r save_changes}
write_csv(full_data_set, "full_filtered_data.csv")
```
