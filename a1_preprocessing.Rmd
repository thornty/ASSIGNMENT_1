...
Demo <- read_csv(file = "demo_train.csv")
LU <- read_csv(file = "LU_train.csv")
Word <- read_csv(file = "token_train.csv")
```
...
Demo <- Demo %>% 
    select(SUBJ, VISIT,Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw,
           Socialization)
LU <- LU %>% 
...
```{r merge_data}
Merge_df <- merge(Demo, LU, all.x=TRUE)
Merge_df <- merge(Merge_df, Word, all.x=TRUE)
```
...
#Make a loop that puts all the 1st visits into another df
visit1 <- tibble() #empty df

for (i in 1:nrow(Merge_df)){
    if (Merge_df[i,2]==1){
        visit1 <- rbind(visit1, Merge_df[i,])}
    }

#Select only the columns needed and rename them:
visit1 <- visit1 %>% 
...
#Merge the df to my original data
Merge_df <-merge(Merge_df, visit1, all.x=TRUE)
```
